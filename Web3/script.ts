...
const HEX_PATTERNS: Record<number, SegmentPattern> = {
...
// BCD Minterms (0-9)
const BCD_MINTERMS: Record<SegmentId, number[]> = {
    a: [0, 2, 3, 5, 6, 7, 8, 9],
    b: [0, 1, 2, 3, 4, 7, 8, 9],
    c: [0, 1, 3, 4, 5, 6, 7, 8, 9],
    d: [0, 2, 3, 5, 6, 8, 9],
...
const BCD_EXPRESSIONS: Record<SegmentId, string> = {
    a: "A + C + B·D + B'·D'",
    b: "B' + C·D + C'·D'",
    c: "B + C' + D",
...
const HEX_EXPRESSIONS: Record<SegmentId, string> = {
    a: "A + C + B·D + B'·D'",
    b: "B' + C'·D' + C·D",
    c: "B + C' + D",
...
const breadcrumbCurrent = document.getElementById("breadcrumbCurrent") as HTMLElement;

const encBcdBtn = document.getElementById("encBcdBtn") as HTMLButtonElement;
const encHexBtn = document.getElementById("encHexBtn") as HTMLButtonElement;
const polCathodeBtn = document.getElementById("polCathodeBtn") as HTMLButtonElement;
const polAnodeBtn = document.getElementById("polAnodeBtn") as HTMLButtonElement;

const segmentDisplay = document.getElementById("segmentDisplay") as HTMLElement;
const reverseMatchText = document.getElementById("reverseMatchText") as HTMLElement;
const bcdInput = document.getElementById("bcdInput") as HTMLElement;
...
const karnaughMaps = document.getElementById("karnaughMaps") as HTMLElement;
const circuitDiagram = document.getElementById("circuitDiagram") as HTMLElement;
const verilogBox = document.getElementById("verilogBox") as HTMLElement;
const copyVerilogBtn = document.getElementById("copyVerilogBtn") as HTMLButtonElement;
const segTimingCanvas = document.getElementById("segTimingCanvas") as HTMLCanvasElement;

const displayHint = document.getElementById("displayHint") as HTMLElement;
const counterSection = document.getElementById("counterSection") as HTMLElement;
...
const counterStepFwd = document.getElementById("counterStepFwd") as HTMLButtonElement;
const counterStepBack = document.getElementById("counterStepBack") as HTMLButtonElement;
...
let currentMode: "interactive" | "counter" = "interactive";
let isHexMode = false;
...
let segmentValues: SegmentPattern = { a: 1, b: 1, c: 1, d: 1, e: 1, f: 1, g: 0 };
...
// Segment Waveform History
interface SegWavePoint {
    time: number;
    segs: Record<SegmentId, number>;
}
let segWaveHistory: SegWavePoint[] = [];
let segWaveTimer = 0;
...
function recordSegmentWave(): void {
    segWaveTimer++;
...
function drawSegTimingDiagram(): void {
    if (!segTimingCanvas || segWaveHistory.length === 0) return;
...
function render7Segment(pattern: SegmentPattern, size = 180): string {
    const w = size;
    const h = size * 1.7;
    const segLen = w * 0.58;
    const gap = w * 0.06;
...
function findMatchingPattern(pat: SegmentPattern): string {
...
function buildInputs(): void {
    ...
    bcdInput.querySelectorAll(".input-toggle-btn").forEach(btn => {
        btn.addEventListener("click", () => {
            ...
            if (currentInput & w) currentInput &= ~w;
            else currentInput |= w;

            const maxLimit = isHexMode ? 15 : 9;
            if (currentInput > maxLimit) currentInput = maxLimit;
...
function syncDisplayFromInput(): void {
    ...
    segmentDisplay.querySelectorAll(".segment-group").forEach(grp => {
        grp.addEventListener("click", () => {
            ...
            if (seg) {
                ...
                if (window.StudioFX) window.StudioFX.click(segmentValues[seg] === 1);
...
function reverseDecodeCustomDisplay(): void {
    segmentDisplay.innerHTML = render7Segment(segmentValues);
    segmentDisplay.querySelectorAll(".segment-group").forEach(grp => {
        grp.addEventListener("click", () => {
            const seg = grp.getAttribute("data-seg") as SegmentId | null;
...
function generateVerilogModule(): string {
...
function buildKarnaughMaps(): void {
...
function renderCircuitDiagram(): void {
